...
export async function shareResponse(activeDid: ActiveDid, shareRequestId: string, jwt: string , urlIssuer: string): Promise<IShareResp> {
    try {
    ...
    return await IssuerApi.shareResponse(did,jwt, shareRequestId, token);   
    } catch (error) {
        return {
            status:'error',
            data: {
